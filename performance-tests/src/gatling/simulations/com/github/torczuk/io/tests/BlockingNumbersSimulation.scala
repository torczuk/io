...
class BlockingNumbersSimulation extends Simulation {

  val httpProtocol = http
    .baseUrl("http://ec2-13-250-38-177.ap-southeast-1.compute.amazonaws.com:8080")

  val addNumberScn = scenario("POST numbers")
    .exec(http("post")
      .post("/api/v1/blocking/numbers")
      .header("Content-Type", "application/json")
      .body(StringBody("""{"value": "10.00"}""")))


  val getStatsScn = scenario("GET stats")
    .exec(http("get stats").get("/api/v1/blocking/numbers"))
