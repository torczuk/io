...
class InMemoryNumberRepository(val clock: Clock,
    ...
    override fun add(number: BigDecimal): Mono<Boolean> {
        val seconds = clock.epochTime()
        val statistic = Statistic(1, number)
        cache.compute(seconds) { _, v ->
            when (v) {
                null -> statistic
                else -> statistic + v
            }
    ...
    override fun getAllLast(duration: Duration): Mono<Statistic> {
        val seconds = clock.epochTime()
        ...
        return Mono.just(Statistic(sum.items, sum.avg()))
    }
}
